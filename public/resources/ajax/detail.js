...
//判断是否已收藏
function checkCollectGame(gameid) {
	if (collected_games.indexOf(parseInt(gameid)) > -1) {
		$("#collected_btn").show();
		$("#collect_btn").hide();
	} else {
		$("#collected_btn").hide();
...
var collected_games = [];
function buildCollectGame(data,callback){
	var collGames = data.collGames;
	for (var idx in collGames) {
		collected_games[collected_games.length] = collGames[idx].id;
	}
	callback();
}
/*----initial--------------------------------------*/
jQuery(document).ready(function($) {
	...
	userinfo(function(data){
		buildCollectGame(data,function(){
			gamedetail(getUrlParam("gameid"));
		});
	});
