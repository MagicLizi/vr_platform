 ...
 * @param callback
 */
gameControl.getGameByList = function(gameIds,callback)
{
    var ids= gameIds.substr(0,gameIds.length - 1);
    if (ids)
    {
        var sql = 'SELECT id,name,description,kvImage FROM game WHERE id in ('+ids+')';
        var c = new command(sql,[]);
        executor.query('platform_vr',c,callback);
    }
    else
    {
        callback();
 ...
 * @returns {*}
 */
gameControl.getGameInList = function(gameId,list)
{
